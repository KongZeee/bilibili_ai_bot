...
class VideoUnderstandingConfig:
    ...
    def __init__(self, raw_config: dict, data_dir: str = "./data"):
        ...
        # 帧拼接：连续帧合并成带编号的九宫格，一次 Vision 请求描述多帧。
        # 开启后抽帧硬顶按 frames_per_tile 放大（64 → 64 * frames_per_tile）。
        self.frame_pack_enabled: bool = bool(va.get("frame_pack_enabled", False))
        try:
            frames_per_tile = int(va.get("frames_per_tile", 4))
        except (TypeError, ValueError):
            frames_per_tile = 4
        self.frames_per_tile: int = max(1, min(frames_per_tile, 9))
        try:
            tile_size = int(va.get("frame_pack_tile_size", 512))
        except (TypeError, ValueError):
            tile_size = 512
        self.frame_pack_tile_size: int = max(256, min(tile_size, 1024))
        ...
        try:
            ...
            max_kf = 32
        pack_factor = self.frames_per_tile if self.frame_pack_enabled else 1
        self.max_keyframes: int = max(1, min(max_kf, 64 * pack_factor))
...
class VideoUnderstandingService:
    ...
    def has_vision(self) -> bool:
        """LLM 是否配置了视觉模型（与 LLMManager.vision_analyze 的回退语义一致）"""
        if not self.llm_manager:
            return False
        ...
        if callable(resolve_vision):
            ...
            if vp and vp.client and vp.model:
                return True
            # vision 路由缺失时，LLMManager.vision_analyze 会回退到 chat
            # provider 的嵌套 vision 配置；此处必须给出一致的结论，否则
            # "仅配置 chat vision" 的账号会被错误地跳过视觉轨。
            resolve_chat = getattr(self.llm_manager, "resolve_chat", None)
            if callable(resolve_chat):
                try:
                    ...
                    cp = None
                if cp is not None:
                    vision_client = getattr(cp, "vision_client", None)
                    vision_model = getattr(cp, "vision_model", None) or getattr(
                        cp, "model", None
                    )
    ...
    async def understand(
        ...
        Args:
            ...
            require_complete_visual: 成功描述的帧比例低于配置的
                vision_min_success_ratio（默认 0.5）时抛出可重试错误。
        ...
        async with global_sem:
            ...
            try:
                ...
                # task_id is generated by this method, so the preprocess work
                # directory is deterministic even though the executor thread is
                # still running. Preserve it (or hand it to the caller) instead
                # of returning a pathless result that leaks the late-arriving
                # artifacts until the next boot.
                timeout_work_dir = os.path.join(cfg.temp_dir, task_id)
                # Unique random task_id path: delete it even if the still-running
                # preprocess thread creates it after this snapshot.
                schedule_cleanup(
                    [timeout_work_dir],
                    delay_seconds=1800,
                    delete_if_appears=True,
                )
                work_dir_out = timeout_work_dir if defer_cleanup else ""
                return {
                    "behavior_log": "",
                    "answer": None,
            ...
            if prep.duration <= 0 or prep.fps <= 0:
                # ffprobe failed: all extracted timestamps would be 0 and the
                # alignment log would be wrong. Degrade explicitly instead.
                logger.warning(
                    "视频 probe 失败 duration=%s fps=%s，降级为元数据分析",
                    prep.duration,
                    prep.fps,
                )
                return {
                    ...
                    "degradation_reason": "probe_failed",
                }
            ...
            try:
                ...
                async def _visual_task():
                    ...
                    return await describe_visual_track(
                        ...
                        executor=executor,
                        frame_pack_enabled=cfg.frame_pack_enabled,
                        frames_per_tile=cfg.frames_per_tile,
                        frame_pack_tile_size=cfg.frame_pack_tile_size,
                    )
                ...
                def _audio_task():
                    if prep.audio_path:
                        result = transcribe_audio(
                            ...
                            api_timeout_seconds=cfg.analysis_timeout_seconds,
                            return_result=True,
                        ...
                        return result
                    if prep.audio_extraction_failed and require_complete_audio:
                        raise ASRTranscriptionError(
                            "ASR_AUDIO_EXTRACTION_FAILED",
                            "video has an audio track but audio extraction failed",
                        )
                ...
                try:
                    ...
                    if not dual_task.done():
                        ...
                        try:
                            ...
                            pass
                    # defer_cleanup 调用方（V6 归档）依赖 work_dir 在证据提交后
                    # 才销毁；超时路径必须把该目录交还给调用方，而不是提前删除。
                    if defer_cleanup:
                        work_dir_out = prep.work_dir
                    else:
                        try:
                            import shutil
    ...
    def shutdown(self) -> None:
        ...
        # 关闭预处理线程池：等待在途的 ffmpeg/抽帧结束（有 preprocess_timeout
        # 上限），避免线程在进程优雅关闭后仍持有文件句柄。
        ...
        # 不在这里 rmtree(temp_dir)：在途任务的 work_dir 由上面的 wait=True
        # 保证已结束，但为防御性仍留给下次启动的孤儿清理兜底。
        logger.info("视频理解服务已关闭（临时目录留给下次启动清理）")
